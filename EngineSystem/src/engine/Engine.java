                    ...
                    if (makeactive < paydivided) {
                        amount=MakeAssignmentHelper(amount,current,exist,ownerShip,currentyaz,loan,makeactive);
                            if (i < sorted.size() - 1)
                                ...
                                current.setAction(currentyaz, Transaction.TransactionType.INCOME, amount, current.getBalance(), (current.getBalance() + amount));
        ...
        if(currentyaz<= pay.getnextPaymentTime()) {
            if (pay.TotalOnePaymentToAllLoaners() <= pay.getOwner().getBalance()) {
                for (Lender i : pay.getLoanerList()) {
                    if (i.getTotalPayback() < i.leftToPay()) {
                    ...
                    throw new NotEnoughMoneyException(pay.getId(), pay.getStatus(),pay.getOwner().getBalance(),pay.TotalOnePaymentToAllLoaners());
                }
        }
        pay.updateStatus(currentyaz);
    }
    ...
    public void PaymentForRisk(LoanDTO riskDTO,Double amount,int currentyaz) throws ExtraPaymentMoneyException {
        ...
        for (Lender a : pay.getLoanerList()) {
            ...
            if (amount >= a.getDebt()) {
                ...
                double intrestamount= a.getDebt()*((double)pay.getInterest()/100);
                ...
                double intrestamount= amount*((double)pay.getInterest()/100);
                a.setPaymentlog(new Payment(onepayment,intrestamount,currentyaz,true));
        ...
        if(amount>0.0){
            pay.getOwner().setAction(currentyaz, Transaction.TransactionType.INCOME,amount,pay.getOwner().getBalance(),pay.getOwner().getBalance()+amount);
            pay.updateStatus(currentyaz);
    ...
    public void PayAll(LoanDTO loanDTO,int currentyaz) throws NotEnoughMoneyException {
        ...
        if(data.leftToPayForTheLoan()<=data.getOwner().getBalance()){
            ...
            for (Lender val: data.getLoanerList()){
                ...
                double intrestamount= val.leftToPay()*((double)data.getInterest()/100);
            ...
            data.updateStatus(currentyaz);
             ...
             data.updateStatus(currentyaz);
            throw new NotEnoughMoneyException(data.getId(), data.getStatus(),data.getOwner().getBalance(),data.leftToPayForTheLoan());
        }
    }
